    ...
    <>
      ...
      {isLoading ? (
        ...
        courseData.length>0 ? ( courseData.map((course, index) => (
          <div className="course_inner" key={index}>
            <div className="course_left">
              <img src={course?.picture} alt="course_pic" className="course_img"></img>
            </div>
            <div className="course_right">
              <div className="course_right1">
                <h3 className="course_mainname n1">
                  {course?.title}
                </h3>
                <p className="course_detail n1">
                {course?.description.length > 200 ? course.description.slice(0, 120) + '...' : course?.description}
                </p>
                <p className="course_authorname n1">Author: {course?.addedBy}</p>
                <p className="course_approxtime n1">Approx. <b>{course?.approximateHours} Hours</b> to complete</p>
              </div>
      ...
      {courseData.length>0 && <button className="course_btn n" onClick={addNewCourse}>
        Add New Course
      </button>}
    </>
  );
